    ...
    ------      
        ...
        ---------------
            ...
            -----------------
                ...
                - At the end of the function, it returns boolean value
                - True is returned if user wants to play again
                - False is returned if user doesn't want to paly again
                - if value other than y or n is entered by a user, it asks user to try again


                based on the requriement, the function is defined as the following:
...
while not exit_game:
    ...
    # initialize game
    num_of_lives = 6
    secret_phrase = hf.create_secret_phrase()
    hidden_phrase = hf.generate_hidden_phrase(secret_phrase)
    hf.clear_screen()
    ...
    while not hf.game_is_over(num_of_lives, hidden_phrase, secret_phrase):
        ...
        made_correct_guess = hf.compare_guess(guess, secret_phrase)
        hf.update_hidden_phrase(made_correct_guess, guess, secret_phrase, hidden_phrase)
        num_of_lives = hf.update_num_of_lives(made_correct_guess, num_of_lives)
        hf.clear_screen()
        hf.show_gallow(num_of_lives)
        hf.show_hidden_phrase(hidden_phrase)
